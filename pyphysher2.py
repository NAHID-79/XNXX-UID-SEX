...
def HiddenEye():
  directory = input("Ingrese la ruta donde desea guardar la herramienta (Ej: /root/Desktop):\n--> ")
  ...
  if input("¿Desea ejecutar la herramienta? (y/n)\n--> ").upper() != "Y":
...
def Modlishka():
  ...
  if input("Se necesita reiniciar su equipo para efectuar los cambios ¿Desea continuar? (y/n)\n--> ").upper() != "Y":
    print("GRACIAS POR UTILIZAR TOOLS-PHISHING").format(END)
    exit(0)
...
if input("¿Acepta utilizar esta herramienta con fines educativos? (y/n)\n--> ").upper() != "Y":
    os.system('clear') 
